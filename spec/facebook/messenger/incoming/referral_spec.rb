...
describe Facebook::Messenger::Incoming::Referral do
  ...
  describe '.param' do
    it 'returns the ref value' do
      expect(subject.param.ref).to eq(payload['referral']['ref'])
    end

    it 'returns the source value' do
      expect(subject.param.source).to eq(payload['referral']['source'])
    end

    it 'returns the type value' do
      expect(subject.param.type).to eq(payload['referral']['type'])
    end
  end
end
