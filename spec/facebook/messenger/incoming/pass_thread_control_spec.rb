...
describe Facebook::Messenger::Incoming::PassThreadControl do
  ...
  subject { Facebook::Messenger::Incoming::PassThreadControl.new(payload) }
  ...
  describe '.thread_control' do
    it 'returns the new_owner_app_id value' do
      expect(subject.thread_control.new_owner_app_id)
        .to eq(payload['pass_thread_control']['new_owner_app_id'])
    end

    it 'returns the metadata value' do
      expect(subject.thread_control.metadata)
        .to eq(payload['pass_thread_control']['metadata'])
    end
  end
end
